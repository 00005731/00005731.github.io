// COMMENTS
   ...
   // I learn it by hard via this link   https://developer.mozilla.org/ru/docs/Web/API/Window/localStorage

   // this comment box work with the local storage, thats why comments are avialable after refresh the page
  ...
  function showWarning () {
    ...
    console.warn('App will not work if local storage is disabled or unsupported.'); //if local storage diseabled, comment box doesnt work, and will show the error message
  } 
  ...
  function getComments() {
    return JSON.parse(localStorage.getItem('comments')) || [];   //local sotrage
  }
  ...
  if (supportsLocalStorage()) {
    ...
    showWarning();  //show warning if ocal storage doesnt support
  }
  ...
  function initApp() {
    ...
    removeAll.addEventListener('click', function() {    // remove button
      clearComments(commentList);
    }, true);
    
    commentForm.addEventListener('submit', function (event) {    //submit
